...
from das.modules.common import print_info, print_success, print_error, print_separator
...
class NmapMerger:
	...
	def xml_to_html(self, merged_xml):
		...
		if xsltproc.exists():
			...
			if P.exists():
				print_success(f'Merged HTML report -> {P.resolve()}')
		else:
			print_error(f'{xsltproc}: binary does not exist, install with "sudo apt install xsltproc -y"')

	def show(self):
		"""
		Print raw Nmap reports in simple text format.
		"""
		total_reports = len({x.stem for x in self.nmap_reports})
		print_info(f'Total reports -> {total_reports}')

		i = 1
		for report in self.nmap_reports:
			if report.suffix == '.nmap':
				print_separator(report.name, prefix=f'{i}/{total_reports}')
	...
	def generate(self):
		...
		if self.output.format in ('oN', 'oA'):
			...
			if sorted_reports:
				...
				if P.exists():
					print_success(f'Merged simple text report -> {P.resolve()}')
			else:
